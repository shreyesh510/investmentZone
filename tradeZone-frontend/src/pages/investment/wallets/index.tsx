...
const WalletCard: React.FC<WalletCardProps> = ({ wallet, isDark, onEdit, onDelete }) => {
  const isBank = wallet.type === 'bank' || (!wallet.type && (wallet.platform || '').toLowerCase().includes('bank'));
...
const WalletsPage = memo(function WalletsPage() {
  ...
  return (
    <div className={`h-full ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'} flex flex-col p-6 overflow-y-auto`}>
